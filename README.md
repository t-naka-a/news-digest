...
| Anthropic Claude API | **約100〜200円/月** | 下記試算参照                                       |
| **合計**            | **約100〜200円/月**  |                                                  |
...
- 実行回数：2回/日 × 30日 = **60回/月**
- 1回あたりのトークン数（記事がある場合）
  - 入力：約1,000トークン（記事本文＋プロンプト）
  - 出力：約800トークン（日本語要約）
- claude-sonnet-4-6 料金：入力 $3/MTok、出力 $15/MTok
  - 入力：60 × 1,000 = 60,000トークン → **$0.18**
  - 出力：60 × 800 = 48,000トークン → **$0.72**
  - 月合計：**約$0.90（≒ 135円）**

> キーワードにマッチする記事が少ない日はほぼ無コスト。上記は記事が毎回見つかる場合の上限目安。
