...
### 自社・競合
`ArkEdge` `Hawkeye 360` `HawkEye360` `Unseenlabs`

### 技術
`VDES` `IoT` `RF` `spectrum` `propulsion` `in-orbit servicing` `autonomy` `AI for space`

### 宇宙ビジネス
`commercial space` `launch market` `LEO economy` `space infrastructure` `new space`
`space supply chain` `launch cadence` `satellite constellation` `space logistics` `launch economics` `maritime`

### DeepTech / 投資
`DeepTech` `hard tech` `frontier tech` `industrial renaissance` `venture scale` `capital intensity` `deep industrials`

### 安全保障・政策
`Defense` `space domain awareness` `national security space` `dual-use technology` `space force`
`industrial policy` `strategic autonomy` `space sovereignty` `安全保障` `防衛`
